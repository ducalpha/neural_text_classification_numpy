...
class Trainer:
    ...
    def fit(self, num_epochs: int = 3):
        ...
        print(
            'Epoch 0, train_accuracy: {:.6f}, validate_accuracy: {:.6f}, train sent-accu: {:.6f}, dev sent-accu: {:.6f}'
...
class TrainPredictManager:
    ...
    def run_with_cuda(self, train_path, dev_path, test_path, model: str):
        label_vocab, char_vocab = self.maybe_load_from_files(train_path, dev_path, test_path,
                                                             Path('label_vocab.pkl'), Path('char_vocab.pkl'))
        # read data.
        data_train, label_train, data_dev, label_dev = \
        ...
        if model == 'torch':
            ...
            raise ValueError('Unrecognized model name {}, must be either "torch" or "tensorflow"'.format(model))
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print('Usage: {} <train_path> <dev_path> <test_path> [only_test] [torch|tensorflow] hypertune'.format(
            sys.argv[0]))
        sys.exit(-1)
    ...
    use_cuda = len(sys.argv) >= 6 and sys.argv[5] == 'torch' or sys.argv[5] == 'tensorflow'
    hypertune = len(sys.argv) >= 7 and sys.argv[6] == 'hypertune'

    train_predict_manager = TrainPredictManager()
    if use_cuda:
        train_predict_manager.run_with_cuda(train_path, dev_path, test_path, model=sys.argv[5])
    else:
