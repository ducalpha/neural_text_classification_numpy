...
class Model:
  ...
  def fit(self, X_train: np.ndarray, y_train: np.ndarray,
    ...
    for epoch in range(num_epoch):
      loss = self._train(X_train, y_train)
      accuracy = self._eval(X_dev, y_dev)
      print('Epoch: {}, loss: {}, eval_accuracy: {}'.format(epoch, loss.item(), accuracy))
...
def test_with_generated_data():
  ...
  # print('X', X)
  # print('y', y)
  model = Model(input_size=X.shape[1], hidden_size=5, num_classes=y.shape[1])
